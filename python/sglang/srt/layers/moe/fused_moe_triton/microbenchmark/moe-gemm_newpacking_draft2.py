...
def moe_gemm_kernel(a_ptr, b_ptr, c_ptr, a_scale_ptr, b_scale_ptr, b_scale_int4_ptr, topk_weights_ptr,
    ...
    if use_int4_w:
        ...
        b_ptrs = (b_ptr + off_experts * stride_be + (offs_k_2[:, None] * stride_bk + offs_bn[None, :] * stride_bn))
    else:
        #load regular
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        if even_Ks:
            ...
            if use_int4_w:
                ...
                b_int4 = b_int4.trans(1,0) ##new packing size ( BLOCK_SIZE_N, BLOCK_SIZE_K /2)
                #bk2 = b_int4.shape[1] *2              
                b = tl.zeros((b_int4.shape[0], BLOCK_SIZE_K), dtype=tl.int8)

                # b[:, ::2] = (b_int4 & 0xF) #AssertionError()
                # b[:, 1::2] = (b_int4 >> 4) & 0xF #AssertionError()
                b= tl.interleave(b_int4, b_int4)

                b = tl.where(b >= 8, b - 16, b)
                b = (b * b_scale_int4).trans(1,0).to(tl.float8e4b8)#reshape to (BLOCK_SIZE_K, BLOCK_SIZE_N) to prepare for dot(a,b)
            else:
                b = tl.load(b_ptrs)
        else:
            a = tl.load(
            ...
            if use_int4_w:
                ...
                b_int4 = tl.load(b_ptrs, mask=(offs_k_2[:, None]) < (K - k) * BLOCK_SIZE_K / 2, other=0.0)
                b_int4 = b_int4.trans(1,0) #size ( BLOCK_SIZE_N, BLOCK_SIZE_K /2)
